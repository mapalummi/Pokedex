function getPokemonCards(pokemon, index, primaryTypeLocal) {
  return `
    <div id="pokeCard${index}" class="new_${primaryTypeLocal}" 
         onclick="showDialogCard(${index}, '${pokemon.name}', '${pokemon.sprites.other.showdown.front_shiny}', ${pokemon.id})">
      <div class="card_content">
        ...
        <div class="character_container">
          ...
          <img src="${pokemon.sprites.front_default}" class="responsive_img" loading="lazy" width="120" height="120">
        </div>
      </div>
    </div>
  `;
...
function getFilteredPokemonCards(pokemon, primaryTypeLocal) {
  return `
    <div id="pokeCard${pokemon.index}" class="new_${primaryTypeLocal}" 
         onclick="showDialogCard(${pokemon.index}, '${pokemon.name}', '${pokemon.sprites.other.showdown.front_shiny}', ${pokemon.id})">
      <div class="card_content">
        <div class="id_container"><p>#${pokemon.id}</p></div>
        <div class="name_container"><p>${pokemon.name}</p></div>
        <div class="character_container">
          ...
          <img src="${pokemon.sprites.front_default}" alt="${pokemon.name}" class="responsive_img" loading="lazy" width="120" height="120">
        </div>
      </div>
    </div>
  `;
